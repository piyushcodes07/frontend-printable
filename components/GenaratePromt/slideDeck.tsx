...
export function SlideDeck({ slides }: { slides: { type: string; content: any }[] }) {
	...
	return (
		<div className="w-full max-h-1/3 flex flex-col items-center my-4">
			<div className="w-full flex justify-center">
				<div className="w-full max-w-[1000px] flex justify-end mb-4">
					<button
						className="px-4 py-2 bg-[#61e987] rounded-md cursor-pointer hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
						type="button"
			...
			<div className="flex flex-col items-center gap-4 mb-4">
				{slides.map((slide, idx) => {
					...
					return (
						<div key={idx} className="h-1/3 w-full max-w-[1000px]">
							<SlideComp
								key={idx}
								{...slide.content}
								backgroundColor={currentTheme?.backgroundColor}
